...
first  characters of her infinite string.
There are  occurrences of a in the substring.

Function Description

Complete the repeatedString function in the editor below.
It should return an integer representing the number of occurrences of
...
1000000000000
Explanation 1
Because all of the first  letters of the infinite string
are a, we print  on a new line.
"""
...
def repeatedString1(s, n):
    # EXAMPLE
    # s = aba; n = 10
    # left_part = ('aba'.count('a')) * (10 // 3) === 6
    # right_part = 'aba'[: 10 % 3].count('a') === 1
    # total = [2 * 10 // 3 + 'aba'[:10 % 3].count('a') == aaaaaa + a] === 7
    slen = len(s)  # = 3
    major = s.count('a') * n // slen
    minor = s[:n % slen].count('a')
    return major + minor
